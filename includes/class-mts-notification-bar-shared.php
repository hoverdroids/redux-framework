...
class MTSNB_Shared {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->plugin_public = new MTSNB_Public( $plugin_name, $version );
	}
	...
	public function ab_test() {
		...
		if ( !is_admin() && !current_user_can('edit_published_posts') && !$this->plugin_public->is_bot() ) {
	...
	public function enqueue_scripts() {
		...
		if ( is_admin() ) {// Needed for Notification Bar preview on admin side
			...
			wp_register_script( 'mtsnb-cookie', MTSNB_PLUGIN_URL . 'public/js/jquery.cookie.js', array( 'jquery' ), $this->version, false );
			...
			if ( $this->bar_id && $this->bar_data ) {
				...
				if ( !empty( $content_type ) ) {
					...
					wp_enqueue_script( 'mtsnb-cookie' );
	...
	public function bar_content( $options ) {
		...
		switch ( $options[$b_prefix.'content_type'] ) {
			...
			case 'button-b':
				...
				$text       = wp_kses_post( $options[$b_prefix.'basic_text'] );
				$link_text  = wp_kses_post( $options[$b_prefix.'basic_link_text'] );
				$link_url   = esc_url( $options[$b_prefix.'basic_link_url'] );
				
				$link_target = '';
				if( isset($options[$b_prefix.'basic_link_target']) && $options[$b_prefix.'basic_link_target'] != '' ) {
					$link_target = '_blank';
				}
				$link_style = $options[$b_prefix.'basic_link_style'];
				echo '<span class="mtsnb-text">'.$text.'</span><a href="'.$link_url.'" class="mtsnb-'.$link_style.'" target="'.esc_attr($link_target).'">'.$link_text.'</a>';

			break;
			case 'posts':
			case 'posts-b':
				$text          = wp_kses_post( $options[$b_prefix.'posts_text'] );
				...
				if ( !empty( $text ) ) echo '<span class="mtsnb-text">'.$text.'</span>';
			...
			case 'newsletter-b':
				...
				if ( 'fb' === $type ) { ?>
					...
					<form class="mtsnb-fb-form" method="post" action="https://feedburner.google.com/fb/a/mailverify" target="mtsnbwindow">
						<?php if ( !empty( $text ) ) { ?>
							<span class="mtsnb-text"><?php echo wp_kses_post( $text ); ?></span>
						<?php } ?>
					...
					<script type="text/javascript">
						jQuery(document).ready(function($){
							$(".mtsnb-fb-form").submit(function(e){
								e.preventDefault();
								window.open('https://feedburner.google.com/fb/a/mailverify?uri=<?php echo esc_attr( $feedburner_id ); ?>', 'mtsnbwindow', 'scrollbars=yes,width=550,height=520');
								return true;
							});
						});
					</script>
			...
			case 'facebook-b':
				...
				if ( !empty( $text ) ) echo '<span class="mtsnb-text">'.wp_kses_post( $text ).'</span>';
			...
			case 'countdown-b':
				...
				if ( !empty( $till_date ) && !empty( $till_time ) ) {
					...
					?>
					<?php if ( !empty( $text ) ) { ?><span class="mtsnb-text"><?php echo wp_kses_post( $text ); ?></span><?php } ?>
					<div class="mtsnb-clock"></div>
					<input type="hidden" class="mtsnb-clock-till" value="<?php echo $count_amount; ?>" />
					<?php if ( !empty( $btn_text ) && !empty( $btn_url ) ) { ?><div class="mtsnb-counter-button-wrap"><a href="<?php echo $btn_url; ?>" class="mtsnb-button"><?php echo wp_kses_post( $btn_text ); ?></a></div><?php } ?>
					<?php
				} else {
			...
			case 'popup-b':
				...
				?>
					<a href="<?php echo esc_url( $link ); ?>" class="mtsnb-text mtsnb-popup-<?php echo $type; ?>"><?php echo wp_kses_post( $text ); ?></a>
				<?php
				} else {
				?>
	...
	public function test_location( $meta_values ) {
		...
		if ( !empty( $meta_values['conditions']['location']['state'] ) ) {
			...
			if (
				...
				$current_url_encoded = esc_url_raw( $current_url );
				...
				if ( !empty( $custom_urls ) ) {
					...
					foreach ( $custom_urls as $custom_url ) {
						...
						if ( trailingslashit( $current_url ) === trailingslashit( $custom_url ) ||
							 $current_url_encoded === $custom_url ||
							 $current_url_encoded === esc_url_raw( $custom_url ) ) {
		...
		if ( !empty( $meta_values['conditions']['notlocation']['state'] ) ) {
			...
			if (
				...
				if ( is_single( $posts_arr ) ) return false;
